...
(function () {
  ...
  function carregarCanais() {
    refs.homeCarregando.classList.remove('oculto');
    refs.homeErro.classList.add('oculto');

    MareApi.buscarCanais().then(function (bruto) {
      refs.homeCarregando.classList.add('oculto');

      estado.canais = normalizarCanais(bruto);
      var agrupado = agruparPorCategoria(estado.canais);
      estado.categoriasOrdem = agrupado.ordem;
      estado.categoriasMapa = agrupado.mapa;
  ...
  function criarTratadorErroLogo(cartaoElemento) {
    return function () {
      ...
      var nomeEl = cartaoElemento.getElementsByClassName('cartao-canal-nome')[0];
      var texto = nomeEl ? nomeEl.textContent : '?';
      cartaoElemento.setAttribute('data-inicial', texto.charAt(0).toUpperCase());
    };
  }
  ...
  function renderizarGradeCanais() {
    ...
    for (var i = 0; i < canais.length; i++) {
      ...
      if (canal.logo) {
        ...
        cartao.setAttribute('data-inicial', canal.nome.charAt(0).toUpperCase());
      }
      ...
      categoriaSpan.className = 'cartao-canal-categoria';
      categoriaSpan.textContent = canal.categoria;
      cartao.appendChild(categoriaSpan);
